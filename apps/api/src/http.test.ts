...
function replyRecorder(queuedHeaders: Record<string, string> = {}) {
...
test("streams a full response without Content-Length", async () => {
  const result = replyRecorder({ "access-control-allow-origin": "http://localhost:3000" });
...
test("uses HTTP/2 frames instead of an HTTP/1 transfer header", async () => {
  ...
  assert.match(result.headers.get("content-disposition") ?? "", /^inline; filename="movie\.mp4"/);
});
